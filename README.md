...
- 근거 검색은 문서를 이미 들고 있는 문서 워커에서 수행합니다. 전체 후보에 BM25-lite 점수를 매긴 뒤 기능별로 관련 근거만 선택합니다. "먼저 자르고 검색"하지 않습니다.
...
- 기준 열이 순번·연번·관리 번호 등이고 기존 값이 연속 번호라면 추가 레코드까지 같은 형식의 연속 번호를 부여합니다. 임의의 식별자는 변경하지 않습니다. 기준 파일의 조건부 서식 규칙은 레코드 범위가 확장된 결과 XLSX에도 유지합니다.
