...
분석은 한 번 실행하면 하나의 흰색 결과 화면에 근거가 연결된 **핵심 요약 → 주요 내용 → 확인된 수치 → 분석 인사이트 → 확인 필요 → 결과 안내** 순서로 표시합니다. 없는 영역은 만들지 않습니다. 주요 내용은 제목·목차·반복 머리말보다 본문 진술과 표의 의미 있는 행을 우선해 최대 7건 선별하고, 같은 근거의 중복 설명을 줄이되 값·조건·파일이 다른 사실은 남깁니다. 확인된 관계가 없으면 인사이트를 표시하지 않고 낮은 확신의 항목은 `확인 필요`에 둡니다. AI가 실패해도 출처가 있는 기본 분석은 유지하며 `기본 분석 완료`와 실패 안내를 마지막에 표시합니다. 별도의 요약 메뉴나 요약 방식 선택은 없습니다.
