...
const failed = () => new ApiError("LAW_MCP_ERROR", "검증·분석 서비스가 요청을 처리하지 못했습니다.", 502);
...
export async function runLegalAnalysis(
  ...
  if (leading.startsWith("[NOT_FOUND]") || leading.startsWith("[LAW_NOT_FOUND]")) {
    return { found: false, mode: request.mode, marker: "NOT_FOUND", text };
  }
  if (leading.startsWith("[INVALID_ARGUMENT]")) {
    return { found: false, mode: request.mode, marker: "INVALID_ARGUMENT", text };
  }
  ...
  if (isError && !(request.mode === "verify_citations" && leading.startsWith("[HALLUCINATION_DETECTED]"))) {
    throw failed();
  }
  const markers = [...new Set(Array.from(text.matchAll(markerPattern), (match) => match[1]))];
  return { found: true, mode: request.mode, text, markers };
}
