 ...
 *
 * The deterministic layers above never call a model. When a local AI endpoint
 * is reachable, its grounded sentence review is folded into the same finding
 * list, always as low-confidence suggestions so a model can never outrank a
 * deterministic rule in the result ordering.
 */
export function semanticFindings(claims: readonly GroundedClaim[]): CheckFinding[] {
  ...
  for (const claim of claims) {
    ...
    findings.push(makeFinding({
      ...
      severity: "suggestion",
      confidence: "low",
      issue: "Local AI 문장 검토 의견",
      message: text,
      reason: claim.kind === "fact"
        ? "Local AI가 원문에서 직접 확인한 문장 문제입니다."
        : "Local AI의 문맥 판단이며 확정된 오류가 아닙니다.",
