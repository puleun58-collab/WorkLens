...
const MCP_TOOL = /\b(?:search_law|get_law_text|get_batch_articles|search_precedents|get_precedent_text|find_similar_precedents|search_decisions|get_decision_text|legal_analysis|legal_research|discover_tools|execute_tool|chain_document_review)\b/u;
...
function withoutOperatorGuidance(line: string): string | undefined {
  if (!OPERATOR_TEXT.test(line)) return line;
  if (SEARCH_ADJUSTMENT.test(line)) return "검색어를 보정해 관련 결과를 찾았습니다.";
  let cleaned = line.replace(TOOL_PARENTHETICAL, "").replace(OMISSION_ARGUMENT, "");
  if (OPERATOR_TEXT.test(cleaned)) {
    cleaned = cleaned
      .split(/(?<=[.。])\s+/u)
      .map((sentence) => sentence.split(/\s+—\s+/u).filter((part) => !OPERATOR_TEXT.test(part)).join(" — "))
