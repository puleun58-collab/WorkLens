...
function taskInstruction(request: AiRequest): string {
  switch (request.operation) {
    ...
    case "analyze":
      return [
        "문서 전체의 목적·핵심 기준·업무 절차·조건과 예외·결론을 먼저 파악하세요. 앞부분과 뒷부분의 상태나 결정이 달라지면 그 변화를 함께 반영하세요. 표의 의미 있는 값은 필요한 경우에만 설명하고, 표지·목차·예시 수치보다 본문을 우선하세요.",
        "summary는 문서 전체가 말하는 바를 근거에 따라 1~4개로 간결하게 정리하세요. 한 부분의 사실을 문서 전체의 결론처럼 쓰지 마세요. 근거가 적으면 하나만 쓰고, 순서가 중요하면 유지하세요.",
        "insight는 두 가지 이상 근거 사이의 실제 조건·결과·우선순위·변화가 드러날 때만 최대 3개 쓰세요. 단일 사실을 반복하거나 summary를 말만 바꿔 쓰지 마세요. 관계가 없으면 쓰지 말고 수를 채우지 마세요.",
        "같은 의미를 여러 claim에 반복하지 말고, 숫자·주체·방향·조건·예외·기한·반올림 규칙이 다른 내용은 뭉뚱그리지 마세요. 원문에 없는 권고·지시·원인·영향·외부 지식을 더하지 마세요.",
        "실제 문서의 주제명이 있으면 section에 쓰고 없으면 빈 문자열을 쓰세요. text와 section은 근거 문서의 주된 서술 언어를 유지하고, 고유명사·전문 용어·숫자·날짜는 원문 표기를 유지하세요. 요약할 내용이 없으면 claims를 빈 배열로 두세요.",
      ].join(" ");
  }
}
