...
function taskInstruction(request: AiRequest): string {
  switch (request.operation) {
    ...
    case "analyze":
      return [
        "문서 전체에서 목적·핵심 기준·업무 절차·조건과 예외·결론을 우선 찾으세요. 표의 의미 있는 값은 필요한 경우에만 설명하고, 표지·목차·예시 수치보다 본문을 우선하세요.",
        "summary는 근거에 뒷받침되는 핵심을 1~4개로 간결하게 쓰세요. 근거가 적으면 하나만 쓰고, 원문에 있는 순서가 중요하면 유지하세요.",
        "insight는 서로 다른 사실 사이의 실제 조건·결과·우선순위·변화를 근거가 뒷받침할 때만 최대 3개 쓰세요. 관계가 없으면 쓰지 말고 수를 채우지 마세요.",
        "같은 내용을 반복하거나 이미 확인되는 수치·사실을 insight로 다시 쓰지 마세요. 원문에 없는 권고·지시·원인·영향·외부 지식을 더하지 마세요.",
