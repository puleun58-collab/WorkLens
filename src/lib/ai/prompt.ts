  ...
  switch (request.operation) {
    ...
    case "brief": {
      ...
      return request.summaryInstruction
        ? `${base}\n\n[사용자 요약 지시사항]\n${request.summaryInstruction}\n\n이 지시는 결과의 형식, 길이, 구조, 독자, 강조점, 출력 언어 또는 명시적 범위만 정합니다. 형식·문체 지시는 검색 키워드로 취급하거나 근거를 임의로 좁히지 말고 문서 전체를 유지하세요. 강조점 요청은 해당 내용을 먼저 배치하되 문서의 다른 핵심을 빼지 마세요. 보고서 형식은 목적 → 핵심 기준 → 주요 흐름·결론 순서의 2~4개 짧은 문단이 되게 하세요. 항목별 정리는 문서의 주요 주제마다 claim을 하나씩 만들고 각 claim의 section에 그 주제 이름을 넣어, 주제 수만큼 항목이 나오게 하세요. 결론·액션 중심 요청에서는 문서의 결론과 주의사항을 summary claim으로 먼저 정리하고, 원문에 명시된 조치가 없으면 action 없이 결론만 제시하세요. 어떤 형식 지시에서도 claims를 비우거나 한 항목으로 뭉치지 마세요. 지시에 다른 출력 언어가 명시된 경우에만 그 언어를 따르세요.`
        : base;
    }
    case "semantic-check":
