  ...
  switch (request.operation) {
    ...
    case "brief":
      return request.instruction
        ? `요약 범위: ${request.instruction}\n이 범위와 직접 관련된 근거만 요약하세요. 관련성이 낮은 일반 정보는 포함하지 말고, 근거가 적으면 짧게 끝내세요. 범위에 맞는 근거가 없으면 claims를 빈 배열로 두세요. 수치와 날짜는 근거 그대로 쓰고 5개 이하 항목으로 정리하세요.`
