import { z } from "zod";
import { extractDocument } from "@/server/deterministic";
...
export async function POST(request: Request) {
  try {
    ...
    if (!parsed.success) throw new WorkspaceError("INVALID_REQUEST", "추출할 파일을 1개 이상 선택하세요.", 400);
    const files = await Promise.all(parsed.data.fileIds.map((id) => getFile(principalKey, id)));
    return ok(await runBoundedOperation(() => ({
      result: files.map((file) => ({ file: { id: file.id, name: file.name }, extraction: extractDocument(file.document) })),
    })));
  } catch (error) {
