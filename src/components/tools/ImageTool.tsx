  ...
  return (
    <div className="image-tool" aria-label="이미지 편집 도구">
      <header className="image-tool-intro">
        <div><p className="image-tool-eyebrow">이미지 · 브라우저 작업공간</p><h2>이미지 편집</h2><p>크기와 영역을 다듬고, 여러 장을 원하는 순서대로 결합하세요. 파일은 이 브라우저 밖으로 전송되지 않습니다.</p></div>
        <span className="image-tool-local">브라우저 내 처리</span>
      </header>
          ...
          <fieldset disabled={!current || busy || mergeActive} className="image-tool-fieldset">
            ...
            <section className="image-tool-control-section">
              ...
              <div className="image-tool-button-row">
                <button type="button" className={mode === "crop" ? "is-chosen" : ""} onClick={() => setMode(mode === "crop" ? null : "crop")}>{mode === "crop" ? "영역 선택 중" : "영역 지정"}</button>
                <button type="button" disabled={!current?.edits.crop} onClick={() => editCurrent(resetCrop)}>자르기 해제</button>
              </div>
            </section>
            <section className="image-tool-control-section"><h4>부분 모자이크</h4><label>블록 크기 <span>{current?.edits.blockSize ?? 12}px</span><input type="range" min="4" max="48" step="2" value={current?.edits.blockSize ?? 12} onChange={(event) => editCurrent((item) => ({ ...item.edits, blockSize: Number(event.target.value) }))} /></label><div className="image-tool-button-row"><button type="button" className={mode === "mosaic" ? "is-chosen" : ""} onClick={() => setMode(mode === "mosaic" ? null : "mosaic")}>{mode === "mosaic" ? "영역 선택 중" : "영역 지정"}</button><button type="button" disabled={!current?.edits.mosaic} onClick={() => editCurrent((item) => ({ ...item.edits, mosaic: null }))}>모자이크 해제</button></div></section>
          </fieldset>
